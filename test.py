...
def test(model, save_path, dataloader, device, parms_path, sparse_th):
    ...
    논문 연결:
      - Section: Granger Causality Discovery + Causality Graph Sparsification 
        + Causal Deviation Scoring
      - 테스트 데이터의 각 윈도우에 대해 인과행렬을 계산하고,
        정상 패턴과의 편차로 이상 점수를 산출
    
    전체 흐름:
      1. 학습된 모델 로드
      2. 테스트 데이터의 각 윈도우에 대해 채널별 그래디언트 계산
      3. 시간축 적분 → 인과행렬 생성
      4. 대칭 제거 (Sparsification)
      5. 임계값 적용
      6. 정상 패턴(Ā_norm)과 비교 → 인과 편차 점수 계산
      7. SMA 스무딩
      8. 평가 지표 계산 (AUROC, AUPRC, F1 등)
    ...
    # ===== 2. 테스트 데이터 순회: 채널별 그래디언트 계산 =====
    for i, (batch_x, batch_y, batch_labels) in pbar:
        """
        batch_x: (batch, seq_len, N) - 입력 윈도우
        batch_y: (batch, pred_len, N) - 타겟
        batch_labels: (batch, seq_len+pred_len) - 이상 라벨
        """
        batch_x, batch_y = batch_x.to(device), batch_y.to(device)
        batch_labels = batch_labels.to(device)
        
        # 윈도우 내 라벨의 최대값 → 윈도우 단위 라벨 (하나라도 이상이면 1)
        label, _ = torch.max(batch_labels, dim=1)

        # 입력에 대한 그래디언트 계산을 위해 requires_grad 설정
        batch_x.requires_grad = True
        outputs = model(batch_x).float().to(device)  # ŷ_t = f(X_{t-1})
        
        # ===== 채널별 그래디언트 계산 (save_train_mean_causal과 동일한 로직) =====
        for features in range(outputs.shape[-1]):
            """
            각 출력 변수 j에 대해:
              1. 채널 j의 예측 오차 L_{t,j} 계산
              2. 역전파하여 ∂L_{t,j}/∂X_t 계산
              3. 절대값 취하기
            """
            model.zero_grad()
            ...
            if features == 0:
                ...
                # grad_causal_mat: (batch, seq_len, N_input, N_output)
                grad_causal_mat = torch.cat(
                    [grad_causal_mat, torch.unsqueeze(Grad_i, dim=3)], dim=3
                )
        ...
        if i == 0:
            ...
            input_grad_causal_mat = torch.cat(
                [input_grad_causal_mat, grad_causal_mat], dim=0
            )
    ...
    """
    논문 Equation 5의 구현:
    시간축(dim=1)에 대한 평균으로 적분 근사
    (samples, seq_len, N, N) → (samples, N, N)
    """
    input_grad_causal_mat = torch.mean(input_grad_causal_mat, dim=1)
    
    # ===== 4. 인과 그래프 희소화 (논문 Equation 6) =====
    """
    Ã_{i,j} = max(0, A_{i,j} - A^T_{i,j}), i ≠ j
    대칭 관계(유사성) 제거 → 단방향 인과관계만 보존
    """
    upper_triangle = torch.triu(input_grad_causal_mat, diagonal=0) 
    lower_triangle_transposed = torch.tril(input_grad_causal_mat, diagonal=-1).transpose(1, 2) 
    result = torch.triu(upper_triangle - lower_triangle_transposed, diagonal=0)
    ...
    # ===== 6. 정상 인과 패턴 로드 =====
    """
    save_train_mean_causal()에서 저장한 Ā_norm 로드
    shape: (N, N) - 정상 상태의 평균 인과행렬
    """
    ...
    # ===== 7. 인과 편차 점수 계산 (Causal Deviation Scoring) =====
    """
    논문 Equation 10: Sc_i = Σ |Ã_{test,i} - Ā_norm| / (Ā_norm + ε)
    
    각 테스트 샘플의 인과행렬과 정상 패턴의 차이를 계산.
    정상 패턴으로 나누어 정규화 → 상대적 편차를 측정.
    
    ε = 1e-4: 0으로 나누는 것을 방지
    
    ★ 이 점수가 높을수록 정상 인과 패턴에서 크게 벗어남 → 이상 가능성 높음 ★
    
    참고: 이 구현에서는 논문의 Sc(인과 편차)와 St(시간 패턴 편차)를 
    분리하지 않고 전체 행렬에 대해 한번에 계산함.
    (대각선 원소가 시간 패턴 정보를 포함하므로, β=0일 때와 유사)
    """
    ...
    "We evaluate using two threshold-independent metrics: 
     the Area Under the Curve (AUC) of the ROC and the PRC."
    """
    
    # AUROC: ROC 곡선 아래 면적 (임계값 독립적 지표)
    auc_score = roc_auc_score(test_labels, err_score)
    print("ROC_score:", auc_score)
    
    # AUPRC: Precision-Recall 곡선 아래 면적 (임계값 독립적 지표)
    ...
    # ===== 10. Point Adjustment (PA) =====
    """
    Point Adjustment: 시계열 이상 탐지에서 널리 사용되는 평가 보정 방법.
    
    아이디어: 이상 구간 내에서 하나라도 맞추면, 해당 구간 전체를 맞춘 것으로 간주.
    이는 실제 운영 환경에서 이상의 시작점을 정확히 맞추는 것보다
    이상 구간을 탐지하는 것이 더 중요하기 때문.
    """
    ...
    for i in range(len(gt)):
        if gt[i] == 1 and pred[i] == 1 and not anomaly_state:
            # 이상 구간 내에서 처음으로 정확히 탐지한 지점
            anomaly_state = True
            # 뒤로 돌아가며 같은 이상 구간의 이전 시점들도 탐지한 것으로 보정
            for j in range(i, 0, -1):
                if gt[j] == 0:
                    ...
                    if pred[j] == 0:
                        pred[j] = 1
            # 앞으로 가며 같은 이상 구간의 이후 시점들도 탐지한 것으로 보정
